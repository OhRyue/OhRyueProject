...
public class AiExplainServiceStub implements AiExplainService {
    ...
    public String explainForQuestion(Question q, Integer chosenIdx, String dbExp) {
        try {
            ...
            log.warn("AI explain failed, fallback to DB exp. qid={}", q != null ? q.getId() : null, e);
    ...
    public String summarizeReview(Map<Long, Boolean> correctness, List<String> weakTags) {
        int total = correctness != null ? correctness.size() : 0;
        long ok = correctness != null ? correctness.values().stream().filter(Boolean::booleanValue).count() : 0;
        long wrong = Math.max(0, total - ok);
        String weak = (weakTags == null || weakTags.isEmpty())
                ? "약점 태그 없음"
                : "약점 태그 예: " + String.join(", ", weakTags);
