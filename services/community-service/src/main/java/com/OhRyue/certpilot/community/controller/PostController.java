...
public class PostController {
  ...
  @PostMapping
  public ResponseEntity<PostDtos.PostSummary> create(@Valid @RequestBody PostDtos.PostCreateRequest request) {
  ...
  /* -------- 내 활동 (페이지네이션) -------- */
  @Operation(summary = "내 커뮤니티 활동 요약", description = "내 게시글은 페이지네이션, 댓글은 최근 N개만 반환합니다.")
  @GetMapping("/my/activity")
  public PostDtos.MyActivityResponse myActivity(@RequestParam(value = "page", defaultValue = "0") int page,
                                                @RequestParam(value = "size", defaultValue = "10") int size,
                                                @RequestParam(value = "commentLimit", defaultValue = "5") int commentLimit) {

    String userId = AuthUserUtil.getCurrentUserId();
    return postService.myActivity(userId, page, size, commentLimit);
  }
