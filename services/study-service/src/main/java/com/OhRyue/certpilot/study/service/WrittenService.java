  ...
  public FlowDtos.StepEnvelope<WrittenDtos.SummaryResp> reviewSummary(Long rootTopicId, Long learningSessionId) {
    ...
    if (sessionId != null && session != null && !Boolean.TRUE.equals(session.getXpGranted())) {
      try {
        ...
        // WRITTEN_REVIEW의 maxXp = 200
        int maxXp = 200;
        double accuracy = mcqTotal > 0 ? (double) mcqCorrect / mcqTotal : 0.0;
        int calculatedEarnedXp = (int) Math.round(maxXp * accuracy);
        
        // 로깅: earnedXp 계산 결과
        System.out.println("[WrittenService.reviewSummary] earnedXp 계산: mcqTotal=" + mcqTotal + 
                          ", mcqCorrect=" + mcqCorrect + ", scorePct=" + scorePct + 
    ...
    if ("MICRO".equals(learningSession != null ? learningSession.getMode() : null)) {
      ...
      if (sessionId != null && session != null && !Boolean.TRUE.equals(session.getXpGranted())) {
        try {
          ...
          // earnedXp 계산 (단일 소스): 정답률에 비례해서 계산
          // WRITTEN_MICRO의 maxXp = 150
          int maxXp = 150;
          double accuracy = totalSolved > 0 ? (double) totalCorrect / totalSolved : 0.0;
          int calculatedEarnedXp = (int) Math.round(maxXp * accuracy);
          
          // 로깅: earnedXp 계산 결과
          System.out.println("[WrittenService.summary] earnedXp 계산 (MICRO): totalSolved=" + totalSolved + 
