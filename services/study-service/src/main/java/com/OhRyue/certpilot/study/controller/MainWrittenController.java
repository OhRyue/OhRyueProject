...
public class MainWrittenController {
    ...
    @Operation(
            ...
            description = "지정한 topicId 범위에서 OX 문제 5문을 무작위로 제공합니다."
    )
    ...
    @Operation(
            summary = "리뷰(총정리) 제출",
            description = "리뷰(총정리) 응답을 제출합니다(내부적으로 MCQ 제출 로직 재사용)."
    )
    @PostMapping("/review/submit")
    public McqSubmitResp reviewSubmit(@Valid @RequestBody McqSubmitReq req) {
        return flow.reviewSubmitWritten(req);
    }
    ...
    public WrongRecapDtos.WrongRecapSet wrongRecapByIds(@RequestParam String ids,
