...
public interface ProgressXpClient {
    ...
    record XpEarnRequest(
            ...
            Integer earnedXp,      // 지급할 XP (단일 소스, 우선 사용). 없으면 scorePct/correctCount/totalCount로 계산
            Double scorePct,       // 선택 (earnedXp가 없을 때만 사용, 정답률 기반 XP 계산용, 0.0 ~ 100.0)
            Integer correctCount,  // 선택 (earnedXp가 없을 때만 사용, 정답 수)
            Integer totalCount     // 선택 (earnedXp가 없을 때만 사용, 총 문제 수)
    ) {}
