...
INSERT INTO question_tag (question_id, tag)
SELECT @q_mcq_access, 'UI'
WHERE @q_mcq_access IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM question_tag WHERE question_id=@q_mcq_access AND tag='UI');

-- 객체지향 리뷰용: 다형성 → 태그를 OOP 로 매핑
INSERT INTO question (cert_id, topic_id, mode, type, difficulty, stem, answer_key, solution_text, source)
...
INSERT INTO question_tag (question_id, tag)
SELECT @q_mcq_poly, 'OOP'
WHERE @q_mcq_poly IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM question_tag WHERE question_id=@q_mcq_poly AND tag='OOP');
